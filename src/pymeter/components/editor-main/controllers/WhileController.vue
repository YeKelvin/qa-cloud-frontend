  ...
  <div class="pymeter-component-container" tabindex="-1">
    ...
    >
      ...
      <el-form-item label="表达式：" prop="elementProps.WhileController__condition">
        ...
        <el-tag
          type="danger"
          style="font-size: 14px; border-top-left-radius: 0; border-top-right-radius: 0"
          disable-transitions
        >
          注意：仅支持单行表达式
        </el-tag>
...
const queryOfflineData = async () => {
  ...
  editorRef.value.setValue(offline.data.elementProps.WhileController__condition)
}
...
const queryBackendData = async () => {
  ...
  editorRef.value.setValue(response.result.elementProps.WhileController__condition)
}
