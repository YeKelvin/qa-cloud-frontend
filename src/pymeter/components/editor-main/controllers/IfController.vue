...
const queryOfflineData = async () => {
  ...
  editorRef.value.setValue(offline.data.elementProps.IfController__condition)
}
...
const queryBackendData = async () => {
  ...
  editorRef.value.setValue(response.result.elementProps.IfController__condition)
}
